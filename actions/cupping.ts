...
import type { Room, RoomPlayer, RoomInvitation, PublicProfile, RoomCoffee, CuppingSession, CuppingSample, CuppingScore, ScaCuppingScores, CuppingFormType, CuppingDashboardData, CuppingSessionDetailData } from '@/types/database'
...
export async function submitCuppingScores(
  roomId: string,
  scores: Array<{ sampleNumber: number; scores: ScaCuppingScores; totalScore: number }>
): Promise<{ success?: boolean; error?: string }> {
  const profile = await getProfileId()
  if (!profile) return { error: 'Not authenticated' }
  const { profileId } = profile
...
export async function getCuppingResults(
  ...
  const scoresWithDetails = (scores || []).map((s) => ({
    ...s,
    scores: s.scores as unknown as ScaCuppingScores,
